...
class ProductExtractionModel(BaseModel):
    ...
    enable_checkout: Optional[str] = Field(
        "false", 
        description="Allows direct purchase inside ChatGPT ('true' or 'false')"
    )
    ...
    link: Optional[str] = Field(
        None,
        description="Product detail page URL (will be generated if not found)"
    )

    # Basic Product Data (Recommended/Optional)
    gtin: Optional[str] = Field(
        None,
        description="Universal product identifier (GTIN, UPC, ISBN) if mentioned"
    )

    mpn: Optional[str] = Field(
        None,
        max_length=70,
        description="Manufacturer part number if mentioned"
    )

    # Item Information
    condition: Optional[str] = Field(
        "new",
        description="Condition of product ('new', 'refurbished', 'used')"
    )
    ...
    image_link: Optional[str] = Field(
        None,
        description="Main product image URL if mentioned or can be inferred"
    )
    ...
    sale_price: Optional[str] = Field(
        None,
        description="Discounted price with currency code if mentioned"
    )
    ...
    inventory_quantity: Optional[int] = Field(
        1,
        description="Stock count (default to 1 if not specified)"
    )
    ...
    item_group_id: Optional[str] = Field(
        None,
        max_length=70,
        description="Variant group ID if product has variants"
    )

    color: Optional[str] = Field(
        None,
        max_length=40,
        description="Variant color if mentioned"
    )

    size: Optional[str] = Field(
        None,
        max_length=20,
        description="Variant size if mentioned"
    )

    gender: Optional[str] = Field(
        None,
        description="Gender target ('male', 'female', 'unisex') if applicable"
    )
    ...
    seller_url: Optional[str] = Field(
        None,
        description="Seller page URL (will be generated if not found)"
    )

    # Returns (Required)
    return_policy: Optional[str] = Field(
        None,
        description="Return policy URL (will be generated if not found)"
    )

    return_window: Optional[int] = Field(
        30,
        description="Days allowed for return (default 30 if not specified)"
    )

    # Performance Signals (Recommended)
    popularity_score: Optional[float] = Field(
        None,
        description="Popularity indicator (0-5 scale) if mentioned"
    )

    # Reviews and Q&A (Recommended)
    product_review_count: Optional[int] = Field(
        None,
        description="Number of product reviews if mentioned"
    )

    product_review_rating: Optional[float] = Field(
        None,
        description="Average review score (0-5 scale) if mentioned"
    )

    # Technical Specifications (Custom for appliances)
    energy_efficiency_class: Optional[str] = Field(
        None,
        description="Energy efficiency rating (e.g., 'A', 'B', 'C') if mentioned"
    )
    ...
    spin_speed: Optional[str] = Field(
        None,
        description="Maximum spin speed (e.g., '0-1400 rpm') if mentioned"
    )
    ...
    voltage: Optional[str] = Field(
        None,
        description="Operating voltage (e.g., '220-240 V') if mentioned"
    )

    frequency: Optional[str] = Field(
        None,
        description="Operating frequency (e.g., '50 Hz') if mentioned"
    )

    programme_duration: Optional[str] = Field(
        None,
        description="Standard programme duration (e.g., '3:55 h') if mentioned"
    )

    load_capacity: Optional[str] = Field(
        None,
        description="Maximum load capacity (e.g., '1-10 kg') if mentioned"
    )
    ...
    source_file: Optional[str] = Field(
        None,
        description="Source PDF file path (added by system)"
    )

    page_count: Optional[int] = Field(
        None,
        description="Number of pages in source document (added by system)"
    )
